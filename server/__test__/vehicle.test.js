const { describe, expect, test, it, beforeAll, afterAll } = require("@jest/globals");
...
describe("Test get data Vehicle endpoint /vehicles", () => {
  ...
  it("Successfully get vehicles (with access_token) without using query filter parameters /vehicles", async function () {
    const response = await request(app).get("/vehicles").set("access_token", access_token);
  ...
  it("Successfully get vehicles with using query filter parameters /vehicles", async function () {
    ...
    const response = await request(app).get(`/vehicles?location=Jakarta&startdate=${startdate}&enddate=${enddate}`);
  ...
  it("fetch vehicles using query startdate and enddate /vehicles", async function () {
    ...
    const response = await request(app).get(`/vehicles?startdate=${startdate}&enddate=${enddate}`);
...
describe("Test add vehicle endpoint /vehicles method POST", () => {
  ...
  it("Failed added a vehicle with empty name field", async function () {
    const response = await request(app).post("/vehicles").set("access_token", access_token).field("name", "").field("CategoryId", "1").field("price", "270000").field("location", "Bandung").attach("image", "./data/testingImage.png");
  ...
  it("Faild added a vehicle without price feild", async function () {
    const response = await request(app).post("/vehicles").set("access_token", access_token).field("name", "Toyota Rush").field("CategoryId", "1").field("location", "Bandung").field("price", "").attach("image", "./data/testingImage.png");
  ...
  it("Faild added a vehicle with empty image feild", async function () {
    const response = await request(app).post("/vehicles").set("access_token", access_token).field("name", "Toyota Rush").field("CategoryId", "1").field("price", "270000").attach("image", "./data/orders.json");
...
describe("Test delete vehicle endpoint /vehicles/:id", () => {
  it("Successfully delete a vehicle", async function () {
    const response = await request(app).delete("/vehicles/2").set("access_token", access_token);

    expect(response.status).toBe(200);
    expect(response.body).toBeInstanceOf(Object);
    expect(response.body).toHaveProperty("message", "Success Delete Vehicle With Id 2");
  });
  it("Failed to delete because the given id params does not exist in the database / is invalid", async function () {
    const response = await request(app).delete("/vehicles/200").set("access_token", access_token);
